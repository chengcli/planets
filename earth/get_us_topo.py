...
def write_topo_to_netcdf(topo_data, filename: str, resolution: str):
    ...
    with Dataset(filename, "w", format="NETCDF4") as ncfile:
        ...
        # Create variables
        topo_var = ncfile.createVariable("topography_3d", "f4", ("x3", "x2", "x1"))
        topo_var.units = "1"
        ...
        # Global attributes
        ncfile.description = "3D topography data loaded from TorchScript"
        ncfile.source = "Generated by write_topo_to_netcdf function"

def create_topo_input(fname: str, resolution: str, nx1: int):
    data = load_white_sand_topo(fname, resolution)
    nx, ny, nz = data.shape
    result = torch.zeros((nx, ny, nx1), dtype=torch.bool)
    result[:, :, :nz] = torch.from_numpy(data)
    print(f"topo 3d data shape = {result.shape}")
    return result
...
def test_create_topo_input():
    fname = "topo_24kmx12km_32.7719_32.9881_-106.5098_-106.3802_adjusted.pt"
    resolution = "40m"
    nx1 = 200

    create_topo_input(fname, resolution, nx1)

if __name__ == "__main__":
    #test_load_white_sand_topo()
    test_create_topo_input()
